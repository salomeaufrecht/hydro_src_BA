...
/*
Measures distance from element.geoetry().center() to points in on border --> refines clodest ones
...
//FieldVector<double, dim> centerHypothenuse(element){
//    FieldVector<double, dim> c1 = element.geometry().corner(0);
//    FieldVector<double, dim> c2 = element.geometry().corner(2);
//    return c1 + (c2-c1)/2;
//}


// element.geometry().corner(0) + (element.geometry().corner(2) - element.geometry().corner(0))/2
...
void horizontalFlow(std::shared_ptr<Dune::ALUGrid< dim, dim, Dune::simplex, Dune::conforming>> grid, double width, FieldVector<double, dim> pos ){
    ...
    for (auto point : points)
    {
        ...
        int c=0;
        while((minDistBorderL > epsilon || minDistBorderU > epsilon) && c<1){
            ...
            for (const auto& element : elements(gridView)){
                ...
                if(std::abs(signedDistance(point+FieldVector<double, dim>{{0, width/2}}, element.geometry().corner(0) + (element.geometry().corner(2) - element.geometry().corner(0))/2)) < minDistBorderU){
                    minDistBorderU = std::abs(signedDistance(point+FieldVector<double, dim>{{0, width/2}}, element.geometry().corner(0) + (element.geometry().corner(2) - element.geometry().corner(0))/2));
                }
                if(std::abs(signedDistance(point-FieldVector<double, dim>{{0, width/2}}, element.geometry().corner(0) + (element.geometry().corner(2) - element.geometry().corner(0))/2)) < minDistBorderL){
                    minDistBorderL = std::abs(signedDistance(point-FieldVector<double, dim>{{0, width/2}},element.geometry().corner(0) + (element.geometry().corner(2) - element.geometry().corner(0))/2));
                }
            }
            ...
            for (const auto& element : elements(gridView)){
                if(std::abs(std::abs(signedDistance(point+FieldVector<double, dim>{{0, width/2}}, element.geometry().corner(0) + (element.geometry().corner(2) - element.geometry().corner(0))/2)) - minDistBorderU) < 0.000001 ||
                    std::abs(std::abs(signedDistance(point-FieldVector<double, dim>{{0, width/2}}, element.geometry().corner(0) + (element.geometry().corner(2) - element.geometry().corner(0))/2)) - minDistBorderL) < 0.00001){
                    Dune::FieldVector<double, 2> u1{1.0, 0.0};
                    Dune::FieldVector<double, 2> u2{0, 1.0};
                    Dune::FieldVector<double, 2> v = element.geometry().corner(2) - element.geometry().corner(0);
                    double dotProduct1 = v[0] * u1[0] + v[1] * u1[1];
                    double dotProduct2 = v[0] * u2[0] + v[1] * u2[1];
                    if (std::abs(dotProduct1) < 1e-8 || std::abs(dotProduct2) < 1e-8 || c<2)  grid->mark(1, element);
                }
            }    
            grid->preAdapt();
            grid->adapt();
        ...
        while((minDistBorderL > epsilon || minDistBorderU > epsilon) && c<50){
            ...
            for (const auto& element : elements(gridView)){
                if(std::abs(std::abs(signedDistance(point+FieldVector<double, dim>{{0, width/2}}, element.geometry().corner(0) + (element.geometry().corner(2) - element.geometry().corner(0))/2)) - minDistBorderU) < 0.000001 ||
                    ...
                    Dune::FieldVector<double, 2> u{1.0, 0.0};
                    Dune::FieldVector<double, 2> v = element.geometry().corner(2) - element.geometry().corner(0);
                    double dotProduct = v[0] * u[0] + v[1] * u[1];
                    if (std::abs(dotProduct) < 1e-8)  grid->mark(0, element);
                }
            }    
            grid->preAdapt();
            grid->adapt();
