...
void horizontalFlow(std::shared_ptr<Dune::ALUGrid< dim, dim, Dune::simplex, Dune::conforming>> grid, double width, FieldVector<double, dim> pos, double minSizeFactor = 0.5 ){
    ...
    double minSize =  minSizeFactor*width;
    std::cout << width << " " << minSizeFactor << std::endl;
    ...
    while( c<100){
        ...
        for (const auto& element : elements(gridView)){
            ...
            for (int i = 0; i < 3; i++){
                ...
                else if (std::abs(element.geometry().corner(i)[1] - yBorderD) < 1e-5 ||std::abs( element.geometry().corner(i)[1] - yBorderU) < 1e-5){ //border on edge of triangle
                    grid->mark(-1, element); 
                    continueFor = true;
                    break;
                }
            ...
            if(xOut == 3 || continueFor || between + upper + under <= 1) continue; // all 3 corners outside x boundry/border onborder not within triangle
            ...
            if (std::min({dist01, dist02}) < minSize) continue; //small enough
            ...
            if(std::abs(yBorderU - yCenterHeight) < epsilon3 || std::abs(yBorderD - yCenterHeight) < epsilon3) continue; // border close to middle of triangle (in right angle)
